...
int main(){
	...
	// write data: 255 logical blocks
	char data1[4096 * 64] = "";
	for(i = 0; i < sizeof(data1); i ++)
		data1[i] = 'w';
	printf("\nWriting LBA 0~255.\n");
	printf("Write cmd return: %d\n",lib_nvme_write(fd, nsid, data1, 255, 0x0));

	// read data
	char data2[4096 * 64] = {0};
	printf("\nReading LBA 0~255.\n");
	printf("read cmd return: %d\n",lib_nvme_read(fd, nsid, data2, 255, 0x0));
	printf("read length: %d\n", strlen(data2));
	printf("a[0]:%c, a[131071]:%c, a[131072]:%c\n", data2[0], data2[131071], data2[131072]);

	// unmap
	printf("\nUnmapping!\n");
	printf("unmap cmd return: %d\n",lib_nvme_unmap(fd, nsid, 0x0, 256));

	// write data: 256 logical blocks
	char data3[4096 * 64] = "";
	for(i = 0; i < sizeof(data3); i ++)
		data3[i] = 'w';
	printf("\nWriting LBA 0~256.\n");
	printf("Write cmd return: %d\n",lib_nvme_write(fd, nsid, data3, 256, 0x0));

	// read again
	char data4[4096 * 64] = "";
	printf("\nReading LBA 0~256.\n");
	printf("read cmd return: %d\n",lib_nvme_read(fd, nsid, data4, 256, 0x0));
	printf("read length: %d\n", strlen(data4));
