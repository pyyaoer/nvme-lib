...
int main(){
	...
	for(i = 0; i < sizeof(data1); i ++)
		data1[i] = 'w';
	printf("\nWriting LBA 0~255.\n");
	printf("Write cmd return: %d\n",lib_nvme_write_core(fd, nsid, data1, 255, 0x0));

	// read data
	char data2[4096 * 64] = {0};
	printf("\nReading LBA 0~255.\n");
	printf("read cmd return: %d\n",lib_nvme_read_core(fd, nsid, data2, 255, 0x0));
	...
	for(i = 0; i < sizeof(data3); i ++)
		data3[i] = 'w';
	printf("\nWriting LBA 0~256.\n");
	printf("Write cmd return: %d\n",lib_nvme_write_core(fd, nsid, data3, 256, 0x0));

	// read again
	char data4[4096 * 64] = "";
	printf("\nReading LBA 0~256.\n");
	printf("read cmd return: %d\n",lib_nvme_read_core(fd, nsid, data4, 256, 0x0));
	printf("read length: %d\n", strlen(data4));
	...
	// write data: 0~512 logical blocks
	char data5[4096 * 128] = "";
