...
int main(){
	...
	for(i = 0; i < sizeof(data1); ++i)
		data1[i] = 'w';
	printf("\nWriting LBA 0~255.\n");
	printf("Write cmd return: %d\n",lib_nvme_write_ioctl(fd, nsid, data1, 256, 0x0));

	// read data
	char data2[4096 * 64] = {0};
	printf("\nReading LBA 0~255.\n");
	printf("read cmd return: %d\n",lib_nvme_read_ioctl(fd, nsid, data2, 256, 0x0));
	...
	for(i = 0; i < sizeof(data3); i ++)
		data3[i] = 'w';
	printf("\nWriting LBA 0~256.\n");
	printf("Write cmd return: %d\n",lib_nvme_write_ioctl(fd, nsid, data3, 257, 0x0));

	// read again
	char data4[4096 * 64] = "";
	printf("\nReading LBA 0~256.\n");
	printf("read cmd return: %d\n",lib_nvme_read_ioctl(fd, nsid, data4, 257, 0x0));
	printf("read length: %d\n", strlen(data4));
	...
	for(i = 0; i < sizeof(data5); i ++)
		data5[i] = 'w';
	printf("\nWriting LBA 0~511.\n");
	printf("Write cmd return: %d\n",lib_nvme_write(fd, nsid, data5, 512, 0x0));

	// read again
	char data6[4096 * 128] = "";
	printf("\nReading LBA 0~511.\n");
	printf("read cmd return: %d\n",lib_nvme_read(fd, nsid, data6, 512, 0x0));
	printf("read length: %d\n", strlen(data6));
